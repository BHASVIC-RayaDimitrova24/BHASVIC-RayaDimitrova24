	...
	public Main() {
		...
		while (!gameOver) {
			for (int p = 0; p < players.size(); p++) {
				...
				board.display();
				//player.makeMove(board); //make with if statements
				if(p==0){
					player.makeMove1(board);
				}
				if(p==1){
					player.makeMove2(board);
				}
				if(p==2){
					player.makeMove3(board);
				}
				if(p==3){
					player.makeMove4(board);
				}
				if(p==4){
					player.makeMove5(board);
				}
				winner = player.checkWinner(board);
				if (winner != "Nobody" ||board.boardFull()) {  //was !=  winner == "Nobody" || board.boardFull()
					gameOver = true;
					break;
				}
			}
